...
def match_and_forward(packet):
    ...
    if IP in packet and UDP in packet:
        ...
        for match in app_server_list: #config['appserver_config']['ar_servers']:
            ...
            elif dst_ip == match['dst_ip'] or dst_ip not in app_not_server_list:  ## it was added to extend the destination ip address control in other!
                message = [f'ACK from {app_name} Server:', (src_ip, dst_ip, src_port, dst_port), str(app_name)]
                send_to_ml_server(message)

            '''elif dst_ip not in config['not_servers']['dst_ips']:
            #if dst_ip in config['not_tuple']['dst_ips'] and dst_port in config['not_tuple']['ports']:
                message = [f'ACK from {app_name} Server: ', (src_ip, dst_ip, src_port, dst_port), str(app_name)]
                send_to_ml_server(message)'''
...
def main():
    global config, ot_server_ip, ot_server_port, app_server_list,app_not_server_list ,app_name
    ...
    app_not_server_list  = config['appserver_config']['not_servers']['dst_ips'] 
